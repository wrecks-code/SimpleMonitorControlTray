...
def get_monitor_selection_list() -> List:
...
def _get_monitor_df() -> pd.DataFrame:
...
def _run_mmt_command(command, destination: str):
...
def disable_monitor(monitor_id: str):
    _run_mmt_command("/disable", monitor_id)


# returns if selected monitor is enabled AND its id
def get_state_and_id_of_monitor() -> tuple:
    df = _get_monitor_df()
    id_condition = (
